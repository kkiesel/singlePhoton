...
class Scan:
    ...
    def getGrid( self ):
        ...
        if yList != range( yList[0], yList[-1]+1, stepSizeY ):
            ...
            print yList
            yList = range( yList[0], yList[-1]+1, stepSizeY )

        defaultTitle = ""
        if len(xList) == 17 and len(yList) == 17:
            defaultTitle = ";m_{#tilde{q}} [GeV];m_{#tilde{g}} [GeV]"
        if len(xList) == 17 and len(yList) == 32:
            defaultTitle = ";m_{#tilde{g}} [GeV];m_{#tilde{#chi}^{0}_{1}} [GeV]"
        if len(xList) == 25 and len(yList) == 32:
            defaultTitle = ";m_{#tilde{g}} [GeV];m_{#tilde{#chi}_{1}} [GeV]"
...
# edge excluded).
# For the simplified model, cross sections are given in pb. The cross section
# uncertainties include pdf(rate) and k-factor uncertainties and are given in percent.
"""%( time.strftime("%Y-%m-%d %H:%M:%S"), filename )

        if "T5gg" in filename:
            pdfUncert = getPdfUncertaintiesAcceptance( "../pdfUncertaintyAcceptance/T5gg.dat" )
        if "T5wg" in filename:
            pdfUncert = getPdfUncertaintiesAcceptance( "../pdfUncertaintyAcceptance/T5wg.dat" )

        #xSections = readSignalXSection( "../../infos/Spectra_gsq_W_8TeV.xsec" )
        if "T5" in filename:
            xSections = readSModelXsection( "../../infos/simplifiedModel.xsec" )
        ...
        for coordinate, histoDict in self.points.iteritems():
            ...
            if coordinate not in pdfUncert:
            ...
            signalCardString += "Point %s cross section = %s\n"%(counter, xsec)
            signalCardString += "Point %s relative total theoretical cross section uncert = %s\n"%(counter, xsecUncert)
...
def meanMet( histoDict ):
    if "gMet" not in histoDict.keys(): return 0
    metClone = histoDict["gMet"].Clone()
    ax=metClone.GetXaxis()
...
def smsPdfUncertAcc( histogram, filename ):
    histogram = histogram.Clone( "pdfUncertAcc" )
    histogram.SetZTitle( "pdf uncert (acc). [%]" )
    pdfUncert = getPdfUncertaintiesAcceptance( filename )
    for multiBin, info in pdfUncert.iteritems():
        bin = histogram.FindBin( multiBin[0], multiBin[1] )
        histogram.SetBinContent( bin, info*100. )
    return [histogram]
...
if __name__ == "__main__":
    ...
    for filename in opts.filenames:
        ...
        if "T5gg" in scanname:
            histos.extend( smsPdfUncertAcc( scan.defaultHisto, "../pdfUncertaintyAcceptance/T5gg.dat") )
        if "T5wg" in scanname:
            histos.extend( smsPdfUncertAcc( scan.defaultHisto, "../pdfUncertaintyAcceptance/T5wg.dat") )
        ...
        for histo in histos:
            ...
            if name == "ht":
                ...
                histo.SetMaximum( 1700 )
            elif name == "nJet":
                histo.SetMinimum( 2 )
                histo.SetMaximum( 6 )
            elif name == "pt":
                histo.SetMinimum( 110 )
                histo.SetMaximum( 850 )
            elif name == "met":
                histo.SetMinimum( 100 )
            elif name == "xSection":
                can.SetLogz()
                histo.SetMinimum(1.79e-4)
                histo.SetMaximum(19)
            elif name == "xSectionUncert":
                histo.SetMinimum(15)
                histo.SetMaximum(47.3)
            ...
            if True:
                ...
                histo.GetZaxis().SetTitleOffset(0.88 )
                histo.Draw("colz")
            ...
            if name in  [ "xSection", "xSectionUncert" ]:
                scanText = "pp#rightarrow#tilde{g}#tilde{g}"
            ...
            largeInfo = ROOT.TLatex()
            largeInfo.SetTextSize(info.GetTextSize()*2)
            largeInfo.DrawLatexNDC( 0.2, 0.8, scanname )
